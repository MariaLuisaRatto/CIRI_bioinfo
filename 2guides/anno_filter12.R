...
# Load 10X data
data <- Read10X_h5("filtered_feature_bc_matrix.h5", unique.features = TRUE)
...
ggsave(p, filename = "./RiboMito.pdf", width = 10, height = 10)
...
ggsave(p, filename = "./RiboMito_filtered.pdf", width = 10, height = 10)
...
gc()                      # garbage collect
rm(list = setdiff(ls(), "mat"))  # remove everything except needed
gc()                      # collect again
...
print(dim(mat))
fwrite(as.data.frame(mat), file = "mingenes_proteincoding_filtered_feature_bc_matrix.csv", row.names = TRUE)
...
assigned_wide = read.csv("./annotation_data.csv")
...
fwrite(mat, file = "annotated_matrix.csv", row.names = TRUE)
